# ![WipeTrash Icon](Burn_Folder_128x128_43380.png Полное руководство  
Комплексное решение для безопасной очистки системного мусора с расширенными возможностями.

> ⚠️ **Внимание!** (04 августа 2025)  
> Данный проект находится в активной разработке и не прошёл полное тестирование.  
...
2. 🔧 Настройка окружения и запуск:
    > 💡 trash_auto.conf (посмотреть и отредактировать если нужно)
    ```bash
    bash setup_wt.sh
    nano trash_auto.conf
    bash wipe_trash.sh
    ```
...
📄 Перед использованием изучите: [browser_paths.md](#документация)

### Процесс работы:
1. Запустите пункт **b** для генерации `browser_cache.conf`
2. Отредактируйте файл (удалите ненужные блоки)
3. Повторный запуск будет использовать изменённую конфигурацию
...
**Кратко — теперь всё содержимое учтено, ссылки работают, пункты и структура выверены, исправлены опечатки, пустые возвраты, заголовкам даны якоря ровно по содержанию.**
